import { CalendarDays, CheckCircle2, CircleAlert, FileText, HelpCircle, ListChecks, Printer, Share2, Stethoscope, Trash2 } from 'lucide-react'
...
export function VisitRecordDetail({ record, onPrepareAgain, onShare, onDeleteTranscript, onDeleteRecord, onBack }: VisitRecordDetailProps) {
  return (
    <div className="flow-page visit-detail-page">
      ...
      {record.reviewSource === 'ai' && <div className="record-source"><SourceBadge>{`진료 대화로 만든 AI 기억 확인${record.reviewModel ? ` · ${record.reviewModel}` : ''}`}</SourceBadge></div>}

      {record.transcript && (
        <section className="record-transcript-card">
          <div><FileText /><h2>글로 바꾼 진료 내용</h2>{record.transcriptionModel && <small>{record.transcriptionModel}</small>}</div>
          <p>{record.transcript}</p>
          <small>기계가 받아 적은 글이라 잘못 들린 부분이 있을 수 있어요. 중요한 의료 정보는 의료진의 안내를 기준으로 확인하세요.</small>
          <button className="button button--quiet" type="button" onClick={onDeleteTranscript}><Trash2 aria-hidden="true" /> 글로 바꾼 내용만 삭제하기</button>
        </section>
      )}
      ...
      <section className="record-actions-card">
        ...
        <ul>{record.actions.length > 0 ? record.actions.map((action) => <li key={action}>{action}</li>) : <li>진료 대화에서 명확히 확인된 실천 항목이 없어요.</li>}</ul>
      </section>

      <div className="detail-actions">
        <button className="button button--primary button--large" type="button" onClick={onShare}><Share2 /> 가족에게 공유하기</button>
        <button className="button button--secondary button--large" type="button" onClick={() => window.print()}><Printer /> 인쇄 · PDF로 저장</button>
        <button className="button button--secondary button--large" type="button" onClick={onPrepareAgain}>이 기록으로 다음 진료 준비하기</button>
