...
import type { FamilyInvitation, FamilyMember, SharePreferences, VisitRecord } from '../types'
...
export function Family({ preferences, onToggleSharing, onPrepareForFamily, onOpenShareSettings, onInviteFamily, members, pendingInvitations, sharedRecords, onCancelInvitation, onOpenSharedVisit }: FamilyProps) {
  return (
    <div className="page-stack family-page">
      ...
      <div className="family-member-list">{members.map((member) => <div className="family-person-card" key={member.id}>
      ...
      <section className="family-action-card">
        <span className="round-icon"><Users /></span><div><p className="eyebrow">다음 진료 D-3</p><h2>어머니의 진료를<br />미리 준비해 드릴까요?</h2><p>전화로 들은 증상을 대신 적고 질문 카드로 보내드릴 수 있어요.</p></div>
